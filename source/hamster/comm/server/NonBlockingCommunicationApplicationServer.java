 ...
 * {@link NonBlockingCommunicationApplication} interface.  As soon as the run() method is called, either by the client thread
 * invoking the run() method or the creation of a new Thread object specifying the instance as its runnable object, the server
 * will call the {@link NonBlockingCommunicationApplication} implementation provided and will register an application controller
...
public final class NonBlockingCommunicationApplicationServer implements Runnable
{
   ...
   */
  private NonBlockingCommunicationApplication application;
   ...
   * 
   * @param sel selector used by the application server.  Passed in so that the constructor does not throw an exception as it may need to be used in a multi-threaded environment.
   ...
   * 
   */
  public NonBlockingCommunicationApplicationServer(Selector sel, NonBlockingCommunicationApplication application, Logger logger)
  {
