...
import hamster.comm.itf.listener.*;
...
/**
 * <p>The {@link NonBlockingCommunicationApplication} class wraps a Java NIO selector loop in a {@link Runnable} object.  The user
 ...
 *
 */
public abstract class NonBlockingCommunicationApplication implements Runnable
{
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  //Application controller - can be used to control listening sockets etc.
  private CommunicationApplicationController controller = new InternalComm();
   ...
   *
   */
  public NonBlockingCommunicationApplication(Logger logger) throws IOException
  {
  ...
  /**
   * Set up the communication application using this callback.  Set up outgoing connection requests and open listening sockets.
   * Set up any internal data structures that are required in the application.
   */
  protected abstract void applicationSetup();

  /**
   * This callback is called by the communications loop when an external shutdown command has been received.  The application
   * should clean itself up, close any open connections and server channels and then call the application controller
   * to get the {@link CommLoopCloser}.  This can be called to exit the communications loop and exit the run() method.
   */
  protected abstract void handleExternalShutdown();

  /**
   * Give the communication the ability to access the communication application controller.
   *
   * @return the controller instance for the communication application.
   */
  protected CommunicationApplicationController accessController()
  {
    return controller;
  }
  ...
  public void run()
  {
    //Call the application setup method.
    applicationSetup();
  ...
  /**
   * This method is designed to be called from outside the communication thread.  It will set the external close flag and
   * wake the selector up.  The loop will then call the application's {@link #handleExternalShutdown()} method to notify
   * it that an external shutdown request has been made.
   */
  ...
  {    
    ...
    public void openClientSocket(InetSocketAddress address, ClientSocketChannelListener connectionListener, SocketChannelOptionAccessor... optionSetters)
    {
      try
      {
        ...
        //Configure socket channel options
        for(SocketChannelOptionAccessor a : optionSetters)
        {
          a.setOption(sc);
        }
